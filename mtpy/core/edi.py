...
def combine_edifiles(fn1, fn2,  merge_freq=None, out_fn = None, 
    ...
    #I) HEAD
    head1 = dict((k.lower(),v) for k,v in list(eo1.head.items()))
    head2 = dict((k.lower(),v) for k,v in list(eo2.head.items()))


    so_headsections = set(list(head1.keys()) + list(head2.keys()))
    ...
    #II) INFO
    info1 = dict((k.lower(),v) for k,v in list(eo1.info_dict.items()))
    info2 = dict((k.lower(),v) for k,v in list(eo2.info_dict.items()))

    so_infosections = set(list(info1.keys()) + list(info2.keys()))
    ...
    #III) DEFINEMEAS
    dmeas1 = dict((k.lower(),v) for k,v in list(eo1.definemeas.items()))
    dmeas2 = dict((k.lower(),v) for k,v in list(eo2.definemeas.items()))

    so_dmeassections = set(list(dmeas1.keys()) + list(dmeas2.keys()))
    ...
    msec1 = dict((k.lower(),v) for k,v in list(eo1.mtsect.items()))
    msec2 = dict((k.lower(),v) for k,v in list(eo2.mtsect.items()))

    so_msecsections = set(list(msec1.keys()) + list(msec2.keys()))
...
def _generate_edifile_string(edidict,use_info_string=False):
    ...
    for sectionhead in lo_sectionheads:
        ...
        if sectionhead == 'HEAD':
            ...
            for k in  sorted(head_dict.keys()):
                v = str(head_dict[k])
        ...
        if sectionhead == 'INFO':
            ...
            info_dict = dict((k.lower(),v) for k,v in list(info_dict.items()))
        ...
        if sectionhead == 'DEFINEMEAS':
            ...
            defm_dict = dict((k.upper(),v) for k,v in list(defm_dict.items()))

            edistring += '>=DEFINEMEAS \n'

            for k in sorted(defm_dict.keys()):
                v = str(defm_dict[k])
                if k == 'REFLAT':
            ...
            if 'REFLAT' not in sorted(defm_dict.keys()):
                v = MTft.convert_degrees2dms_tuple(edidict['HEAD']['lat'])
                edistring += '\tREFLAT={0}:{1}:{2:.2f}\n'.format(int(v[0]),int(v[1]),v[2])
            if 'REFLONG' not in sorted(defm_dict.keys()):
                v = MTft.convert_degrees2dms_tuple(edidict['HEAD']['long'])
                edistring += '\tREFLONG={0}:{1}:{2:.2f}\n'.format(int(v[0]),int(v[1]),v[2])
            if 'REFELEV' not in sorted(defm_dict.keys()):
        ...
        if sectionhead == 'MTSECT':
            ...
            mtsct_dict = dict((k.upper(),v) for k,v in list(mtsct_dict.items()))

            edistring += '>=MTSECT \n'

            for k in sorted(mtsct_dict.keys()):
                v = str(mtsct_dict[k])
                if len(v) == 0 or len(v.split()) > 1:
                    edistring += '\t%s=""\n'%(k)
                else:
        ...
        if sectionhead == 'TIPPER' and ('TIPPER' in edidict):
