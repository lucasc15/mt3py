...
#        print key+': '+str(prepdict[key])
    ...
    for pkey in list(prepdict.keys()):
...
#        bfpath=os.path.join(bfpath,'Run{0}'.format(nruns+1))
    ...
    try:
        ...
        magtype=eval(input('input magnetometer type broadband (bb), long period(lp)'))
...
def writeedi(dirpath,station,stationinfofile=None,rrstation=None,
    ...
    try: 
        ...
        magtype=eval(input('Enter magtype bb for broadband, lp for longperiod. '+\
                    'be sure to put quotes around answer. \n'))
        statdict['lat']=eval(input('Enter latitude. \n'))
        statdict['long']=eval(input('Enter longitude. \n'))
        statdict['ex']=str(eval(input('Enter Ex length (m). \n')))
        statdict['ey']=str(eval(input('Enter Ey length (m). \n')))
        statdict['elev']=str(eval(input('Enter Elevation (m). \n')))
        statdict['egain']=str(eval(input('Enter interface box gain (10 or 100). \n')))
    try:
        dlgain=statdict['dlgain']
    except KeyError:
        dlgain=eval(input('Enter data logger gain (verylow=2.5,low=1,high=.1)'+\
                    'be sure to put quotes around answer. \n'))
