...
#            self.cbar = cbar
    ...
    def plot_interface(self,x,y,z,scale='km'):
        ...
        from . import pek1dplotting as p1dp
...
#            self.figsize=(10,10*ar)
        ...
        if plot_aniso:
            sp = list(range(2,len(self.xyzfiles)+2))
            self.plot_aniso_depth_map()
            title = "Magnitude and depth of anisotropy\nfrom 1D anisotropic inversions"
            self._update_axis_params(title,ad_labels)
        else:
            sp = list(range(1,len(self.xyzfiles)+1))
...
#            print self.xyzfiles[s]
    ...
    def add_cbar(self):
        ...
        cticks = list(range(int(self.levels[0]),int(self.levels[-1]+1)))
        cbar.set_ticks(cticks)

    def add_xy_data(self,spn):
        
        if str(spn) in list(self.additional_xy_data.keys()):
    ...
    def add_ax_text(self,spn):
        ...
        if str(spn) in list(self.plot_text.keys()):
